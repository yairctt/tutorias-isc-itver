...
export async function enviarConstancia(noControl, pdfBuffer) {
  ...
  const anio = `20${noControl.slice(0, 2)}`;
  ...
  const { error } = await resend.emails.send({
    ...
    attachments: [
      {
        filename: `Constancia_Tutoria_ISC_${noControl}.pdf`,  // nombre descriptivo
        content: pdfBuffer,
...
function buildText(noControl) {
  return [
    ...
    `Hola, ${noControl}.`,
    '',
    'Tu constancia de acreditación del Programa de Tutorías está adjunta en PDF.',
